...
// UnifiedSpectralRenderer was replaced with enhanced GPUPixelRenderer which supports emission
// SpectralCalculator import removed - GPU renderer now handles all spectral calculations
...
// Advanced spectral rendering imports
import {
  BlackBodyEmission,
  FluorescenceEmission,
} from "../spectral/emission";
import {
  ScatteringProperties,
} from "../spectral/scattering";
import {
  LayerCompositor,
  MaterialLayer,
} from "../spectral/layers";
import { MaterialShape, MaterialShapeConfig } from "../spectral/layers/MaterialLayer";
import { EmissionProperties } from "../spectral/emission/EmissionSpectrum";
import { FluorescenceConfig } from "../spectral/emission/FluorescenceEmission";

/**
 * Advanced Spectral Coloring Demo
 * Demonstrates physics-based spectral coloring with advanced rendering:
 * - Black body radiation (thermal glow)
 * - Fluorescence emission (UV-excited)
 * - Emission auras
 * - Scattering blur
 * - Multi-layer composition
 */
export class AdvancedSpectralDemo implements Demo {
  readonly name = "Advanced Spectral Coloring";
  readonly description =
    "Physics-based spectral absorption, emission, and scattering with advanced multi-layer rendering";
  ...
  // Advanced spectral rendering components
  private layerCompositor: LayerCompositor;
  private blackBodyEmission: BlackBodyEmission;
  private fluorescenceEmission: FluorescenceEmission;
  
  // (SpectralCalculator removed - GPU renderer now handles all spectral calculations)
  
  // Material shapes for advanced rendering
  private squareShape: MaterialShape | null = null;
  private circleShape: MaterialShape | null = null;
  private triangleShape: MaterialShape | null = null;
  ...
  constructor() {
    ...
    // Initialize advanced spectral rendering components
    this.layerCompositor = new LayerCompositor();
    this.blackBodyEmission = new BlackBodyEmission();
    
    // Default fluorescence configuration (sodium D-line like)
    const defaultFluorescenceConfig: FluorescenceConfig = {
      id: 'sodium-like',
      name: 'Sodium-like Fluorescence',
      excitationPeak: 330, // UV excitation
      excitationWidth: 50,
      emissionPeak: 589, // Sodium D-line
      emissionWidth: 20,
      quantumYield: 0.5,
    };
    this.fluorescenceEmission = new FluorescenceEmission(defaultFluorescenceConfig);
  }
  ...
  initialize(scene: GameScene): void {
    ...
    // Initialize unified spectral renderer with emission support
    const camera = scene.cameras.main;
    // GPUPixelRenderer is already initialized via gpuRenderer field
    // The enhanced GPUPixelRenderer now supports:
    // - Temperature-based emission via Kirchhoff's law
    // - Spectrum readback mode for synchronized plotting
    ...
    // Initialize spectral display with default position (center of visible area)
    // Note: camera variable already declared above
    const defaultX = camera.scrollX + camera.width / 2;
    const defaultY = camera.scrollY + camera.height / 2;
    ...
    this.updateSpectralDisplayAtPoint(scene, defaultX, defaultY);
  ...
  private lockSpectralDisplay(
    ...
    if ((inSquare || inCircle || inTriangle || inBackground) && !inUI) {
      ...
      // Calculate and display spectrum at this position using unified helper
      this.updateSpectralDisplayAtPoint(scene, worldX, worldY);
    }
  }
  ...
  private updateMouseHover(
    ...
    if ((inSquare || inCircle || inTriangle || inBackground) && !inUI) {
      ...
      // Calculate spectrum at this position using unified helper
      this.updateSpectralDisplayAtPoint(scene, worldX, worldY);
  ...
  private updateSpectralDisplayWithCurrentState(scene: GameScene): void {
    ...
    const state = this.lockedSpectralState.locked
      ? this.lockedSpectralState
      : this.currentSpectralState;
    this.updateSpectralDisplayAtPoint(scene, state.worldX, state.worldY);
  }
  
  /**
   * New unified method: Update spectral display using world coordinates.
   * Uses the LayerSystem to apply filters exactly as the renderer does.
   * This ensures feature parity between rendering and spectral display.
   */
  private updateSpectralDisplayAtPoint(scene: GameScene, worldX: number, worldY: number): void {
    // Get shape containment info for display purposes
    const inSquare =
      worldX >= this.state.squareX - this.state.squareSize / 2 &&
      worldX <= this.state.squareX + this.state.squareSize / 2 &&
      worldY >= this.state.squareY - this.state.squareSize / 2 &&
    ...
    // Calculate spectrum using the UNIFIED helper (same as renderer)
    const spectrum = this.calculateSpectrumAtPoint(worldX, worldY, 'display');

    // Draw graph - emission is now already included in the spectrum
    this.drawSpectrumGraph(
      ...
      inBackground,
      onGridLine,
      undefined // No separate emission - it's unified now
    );
  }
  ...
  /**
   * Calculate dark mode background spectrum (high resolution)
   * Zero at all wavelengths - shows only emission
   */
  private calculateDarkBackgroundSpectrum(): SpectrumPoint[] {
    return calculateDarkBackgroundSpectrum();
  }

  /**
   * Calculate dark mode background spectrum (low resolution)
   * Zero at all wavelengths - shows only emission
   */
  private calculateDarkRGBBackgroundSpectrum(): SpectrumPoint[] {
    return calculateDarkRGBBackgroundSpectrum();
  }

  /**
   * Calculate spectrum at a point using the LayerSystem filters.
   * This is the UNIFIED method that both rendering and spectral display should use
   * to ensure feature parity (emission, scattering, etc. are always the same).
   * 
   * @param x World X coordinate
   * @param y World Y coordinate
   * @param resolution Number of wavelength samples (high for display, low for rendering)
   * @returns Combined spectrum with transmission AND emission
   */
  private calculateSpectrumAtPoint(
    x: number,
    y: number,
    resolution: 'display' | 'render' = 'display'
  ): SpectrumPoint[] {
    // Note: GPU renderer doesn't have getSpectrumAtPixel method
    // Spectrum calculation is done via CPU path using same spectral math
    
    // Get background spectrum at requested resolution based on mode
    let spectrum: SpectrumPoint[];
    if (this.state.darkMode) {
      // Dark mode: zero background (shows only emission)
      spectrum = resolution === 'display'
        ? this.calculateDarkBackgroundSpectrum()
        : this.calculateDarkRGBBackgroundSpectrum();
    } else if (this.state.uvMode) {
      spectrum = resolution === 'display'
        ? this.calculateUVBackgroundSpectrum()
        : this.calculateUVRGBBackgroundSpectrum();
    } else {
      spectrum = resolution === 'display'
        ? this.calculateUniformBackgroundSpectrum()
        : this.calculateRGBBackgroundSpectrum();
    }
    
    // Check if point is on grid line and apply 60% intensity reduction
    const onGridLine = this.grid.isOnGridLine(x, y, 2.0);
    ...
    // Get filters from LayerSystem - this includes MaterialFilter which now has emission
    const filters = this.layerSystem.getFiltersForPixel(x, y);
    
    // Apply each filter in order (background → shapes)
    // MaterialFilter now applies both transmission AND emission
  ...
  private drawSpectrumGraph(
    ...
    _emissionSpectrum?: Array<{ wavelength: number; transmission: number }> // Deprecated: emission now unified
  ): void {
    const { width, height } = scene.cameras.main;
    const displayWidth = 400;
    const displayHeight = 300;
    ...
    // Note: Emission is now unified with transmission in the main spectrum line
    // The MaterialFilter applies both absorption AND emission in a single pass
    ...
    // Calculate average intensity (transmission + emission combined)
    const avgIntensity =
      spectrum.reduce((sum, p) => sum + p.transmission, 0) / spectrum.length;
    infoText += `\nAvg Intensity: ${avgIntensity.toFixed(3)}`;
    
    // Note: Emission is now included in the spectrum via MaterialFilter
    // No separate indicator needed - if emission is present, avgIntensity will be higher
  ...
  private calculateNormalizedColors(): {
    ...
    // Get background spectrum based on mode
    const backgroundSpectrum = this.state.darkMode
      ? this.calculateDarkRGBBackgroundSpectrum()
      : this.state.uvMode
        ? this.calculateUVRGBBackgroundSpectrum()
        : this.calculateRGBBackgroundSpectrum();
    ...
    const illuminant = this.state.darkMode ? "D65" : (this.state.uvMode ? "UV" : "D65");
  ...
  private setupLayerSystem(): void {
    ...
    // Also set up the advanced LayerCompositor with MaterialLayers
    this.setupAdvancedLayerSystem();
    
    // Note: GPU renderer temperatures and background modes are handled through 
    // updateMaterialTextures() which is called in renderPixels()
  }
  
  /**
   * Set up the advanced LayerCompositor with MaterialLayers that support emission and scattering
   */
  private setupAdvancedLayerSystem(): void {
    this.layerCompositor.clear();
    
    // Get background spectrum for emission calculations
    const backgroundSpectrum = this.state.darkMode
      ? this.calculateDarkRGBBackgroundSpectrum()
      : this.state.uvMode
        ? this.calculateUVRGBBackgroundSpectrum()
    ...
    // Square layer (water material) - zOrder 1
    const squareAdvancedLayer = new MaterialLayer({ id: "square-advanced", zOrder: 1 });
    const squareMaterial = this.materialRegistry.get("water");
    if (squareMaterial) {
      const squareGeometry = new RectangleGeometry(
        this.state.squareX,
      ...
      // Calculate absorption spectrum from material
      const absorptionSpectrum = this.renderer.calculateFullSpectrum(
        squareMaterial,
        this.state.squareProperties
      );
      
      // Calculate emission spectrum (black body if hot enough)
      const baseEmissionSpectrum = this.calculateEmissionSpectrum(
        this.state.squareProperties.temperature,
        backgroundSpectrum
      );
      
      // Create MaterialShape with full spectral properties
      const squareShapeConfig: MaterialShapeConfig = {
        id: "square-shape",
        geometry: squareGeometry,
        absorptionSpectrum: absorptionSpectrum,
        baseEmissionSpectrum: baseEmissionSpectrum,
        scattering: this.getScatteringForMaterial("water", this.state.squareProperties),
        emissionProperties: this.getEmissionPropertiesForMaterial("water", this.state.squareProperties),
        depth: this.state.squareProperties.depth,
      };
      this.squareShape = new MaterialShape(squareShapeConfig);
      squareAdvancedLayer.addShape(this.squareShape);
    }
    this.layerCompositor.addLayer(squareAdvancedLayer);
    
    // Circle layer (crystal material) - zOrder 2
    const circleAdvancedLayer = new MaterialLayer({ id: "circle-advanced", zOrder: 2 });
    const circleMaterial = this.materialRegistry.get("crystal");
    if (circleMaterial) {
      const circleGeometry = new CircleGeometry(
        this.state.circleX,
      ...
      const baseEmissionSpectrum = this.calculateEmissionSpectrum(
        this.state.circleProperties.temperature,
        backgroundSpectrum
      );
      
      const circleShapeConfig: MaterialShapeConfig = {
        id: "circle-shape",
        geometry: circleGeometry,
        absorptionSpectrum: absorptionSpectrum,
        baseEmissionSpectrum: baseEmissionSpectrum,
        scattering: this.getScatteringForMaterial("crystal", this.state.circleProperties),
        emissionProperties: this.getEmissionPropertiesForMaterial("crystal", this.state.circleProperties),
        depth: this.state.circleProperties.depth,
      };
      this.circleShape = new MaterialShape(circleShapeConfig);
      circleAdvancedLayer.addShape(this.circleShape);
    }
    this.layerCompositor.addLayer(circleAdvancedLayer);
    
    // Triangle layer (gas material) - zOrder 3
    const triangleAdvancedLayer = new MaterialLayer({ id: "triangle-advanced", zOrder: 3 });
    const triangleMaterial = this.materialRegistry.get("gas");
    if (triangleMaterial) {
      const triangleGeometry = new TriangleGeometry(
        this.state.triangleX,
      ...
      const baseEmissionSpectrum = this.calculateEmissionSpectrum(
        this.state.triangleProperties.temperature,
        backgroundSpectrum
      );
      
      const triangleShapeConfig: MaterialShapeConfig = {
        id: "triangle-shape",
        geometry: triangleGeometry,
        absorptionSpectrum: absorptionSpectrum,
        baseEmissionSpectrum: baseEmissionSpectrum,
        scattering: this.getScatteringForMaterial("gas", this.state.triangleProperties),
        emissionProperties: this.getEmissionPropertiesForMaterial("gas", this.state.triangleProperties),
        depth: this.state.triangleProperties.depth,
      };
      this.triangleShape = new MaterialShape(triangleShapeConfig);
      triangleAdvancedLayer.addShape(this.triangleShape);
    }
    this.layerCompositor.addLayer(triangleAdvancedLayer);
  }
  
  /**
   * Calculate emission spectrum based on temperature (black body radiation)
   * and UV excitation (fluorescence)
   */
  private calculateEmissionSpectrum(
    temperature: number,
    excitationSpectrum: SpectrumPoint[]
  ): SpectrumPoint[] {
    const emissionPoints: SpectrumPoint[] = [];
    
    // Check if black body emission is active (temperature > Draper point ~798K)
    const hasBlackBodyEmission = this.blackBodyEmission.isActive(temperature);
    
    // Check if UV mode is on for fluorescence
    const hasFluorescence = this.state.uvMode;
    
    // Generate spectrum points
    for (let wl = 380; wl <= 700; wl += 5) {
      let emission = 0;
      
      // Add black body emission if temperature is high enough
      if (hasBlackBodyEmission) {
        emission += this.blackBodyEmission.getIntensityAt(wl, temperature);
      }
      
      // Add fluorescence emission if UV mode is active
      if (hasFluorescence) {
        // Find UV excitation intensity (200-400nm range)
        const uvIntensity = excitationSpectrum
          .filter(p => p.wavelength >= 200 && p.wavelength <= 400)
          .reduce((sum, p) => sum + p.transmission, 0) / 40; // Average over UV range
        
        // Simple fluorescence model: UV excites visible emission with Stokes shift
        // Peak emission is shifted to longer wavelengths
        const fluorescenceEmission = this.fluorescenceEmission.getEmissionAt(wl, uvIntensity);
        emission += fluorescenceEmission;
      }
      
      emissionPoints.push({ wavelength: wl, transmission: emission });
    }
  ...
  /**
   * Get scattering properties for a material based on its type and solution properties
   */
  private getScatteringForMaterial(
    materialId: string,
    properties: SolutionProperties
  ): ScatteringProperties {
    // Base scattering on particle density and bubble density
    const particleContribution = (properties.particleDensity || 0) * 0.5;
    const bubbleContribution = (properties.bubbleDensity || 0) * 0.3;
    const coefficient = particleContribution + bubbleContribution;
    
    // Different materials have different scattering characteristics
    let wavelengthPower = 0; // Mie by default
    let asymmetry = 0.5; // Moderate forward scattering
    
    if (materialId === "water") {
      // Water with particles: mix of Mie (large particles) and Rayleigh (molecules)
      wavelengthPower = 2; // Between Mie (0) and Rayleigh (4)
      asymmetry = 0.6;
    } else if (materialId === "gas") {
      // Gas: more Rayleigh-like scattering
      wavelengthPower = 4;
      asymmetry = 0.1; // More isotropic
    } else if (materialId === "crystal") {
      // Crystal: depends on inclusions
      wavelengthPower = 0; // Mie from inclusions
      asymmetry = 0.8; // Strong forward scattering
    }
  ...
  /**
   * Get emission properties (aura radius and decay) for a material
   */
  private getEmissionPropertiesForMaterial(
    _materialId: string,
    properties: SolutionProperties
  ): EmissionProperties {
    // Emission aura is stronger for hotter materials
    const temperatureFactor = Math.max(0, (properties.temperature - 798) / 1000);
    
    return {
      auraRadius: 20 + temperatureFactor * 30, // 20-50 pixels
      auraDecay: 0.1 - temperatureFactor * 0.05, // 0.1-0.05 (slower decay when hotter)
      emissivity: 1.0,
    };
  }
  ...
  private renderPixels(scene: GameScene, force: boolean = false): void {
    ...
    try {
      ...
      if (!camera || camera.worldView.width <= 0 || camera.worldView.height <= 0) {
        // Camera not ready yet - keep state dirty so next call will try again
      ...
      // Camera and bounds are valid - mark state as clean before rendering
      ...
      // The enhanced GPUPixelRenderer now supports temperature-based emission
  ...
  private createUIControls(scene: GameScene): void {
    ...
    // Dark Mode toggle - position at top right
    const darkModeButton = scene.add.text(width - 280, 10, "Dark Mode: OFF", {
      fontSize: "14px",
      color: "#000000",
      backgroundColor: "#cccccc",
      padding: { x: 8, y: 4 },
    });
    darkModeButton.setOrigin(0, 0);
    darkModeButton.setDepth(1001);
    darkModeButton.setScrollFactor(0);
    darkModeButton.setInteractive({ useHandCursor: true });
    darkModeButton.on("pointerdown", () => {
      this.state.darkMode = !this.state.darkMode;
      // Dark mode overrides UV mode
      if (this.state.darkMode) {
        this.state.uvMode = false;
        uvModeButton.setText("UV Mode: OFF");
        uvModeButton.setBackgroundColor("#cccccc");
      }
      this.state.markDirty();
      darkModeButton.setText(`Dark Mode: ${this.state.darkMode ? "ON" : "OFF"}`);
      darkModeButton.setBackgroundColor(this.state.darkMode ? "#333333" : "#cccccc");
      darkModeButton.setColor(this.state.darkMode ? "#ffffff" : "#000000");
      
      // Note: GPU renderer background mode is handled via updateMaterialTextures() in renderPixels()
      
      // Re-render pixels with new mode
      this.setupLayerSystem();
      this.renderPixels(scene);
      // Update spectral display with current state
      this.updateSpectralDisplayWithCurrentState(scene);
    });
    this.uiElements.push(darkModeButton);

    // UV Mode toggle - position next to dark mode
    ...
    uvModeButton.on("pointerdown", () => {
      ...
      // UV mode disables dark mode
      if (this.state.uvMode) {
        this.state.darkMode = false;
        darkModeButton.setText("Dark Mode: OFF");
        darkModeButton.setBackgroundColor("#cccccc");
        darkModeButton.setColor("#000000");
      }
  ...
  private createControlPanel(
    ...
    material.molecules.forEach((molecule) => {
      ...
      const updateSlider = (pointer: Phaser.Input.Pointer) => {
        ...
        // GPU renderer will get updated transmission via updateMaterialTextures
        // when renderPixels is called via onUpdate()
    ...
    // Temperature range: 1 K to 10000 K (logarithmic)
    // Extends past sun surface temp (5778K) for super-hot materials
    const minTemp = 1;
    const maxTemp = 10000;
    ...
    const updateTemp = (pointer: Phaser.Input.Pointer) => {
      ...
      // Note: GPU renderer temperature is handled via updateMaterialTextures() in renderPixels()
      // which is called via onUpdate()
  ...
  /**
   * Update method called every frame by GameScene
   * Ensures initial render happens even if camera wasn't ready during initialize
   */
  update(scene: GameScene): void {
    // Check if state is dirty (needs render) and try to render
    // This catches the case where initial render was skipped due to camera not being ready
    if (this.state.checkDirtyState()) {
      this.renderPixels(scene);
    }
  }
