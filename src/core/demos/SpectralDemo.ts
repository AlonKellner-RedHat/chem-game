...
import type { BackgroundMode, BacklightMode } from '../physics/config';
import type { GPUShape, GPUTextureShape } from '../rendering';
...
export class SpectralDemo implements Demo {
  ...
  protected enableBacklight = false;

  // Shapes
  protected shapes: ShapeConfig[] = [];
  protected backgroundMode: BackgroundMode = 'normal';
  protected backlightMode: BacklightMode = 'dark'; // Default to dark (off)
  ...
  protected backlightModeRadio: RadioButtonGroup<BacklightMode> | null = null;
  ...
  cleanup(scene: GameScene): void {
    ...
    this.backlightModeRadio?.destroy();
    this.backlightModeRadio = null;
  ...
  protected createUI(scene: GameScene): void {
    ...
    // Backlight mode radio buttons (independent from ambient)
    if (this.enableBacklight) {
      const backlightOptions: Array<{ value: BacklightMode; label: string }> = [
        { value: 'dark', label: 'Dark' },
        { value: 'normal', label: 'Visible' },
        { value: 'uv', label: 'UV' },
      ];

      this.backlightModeRadio = new RadioButtonGroup<BacklightMode>(buttonContainer, {
        name: 'backlight-mode',
        options: backlightOptions,
        selectedValue: 'dark', // Default to dark (off)
        label: 'Backlight',
        onChange: (value) => {
  ...
  protected async updateRenderer(scene: GameScene): Promise<void> {
    ...
    // Configure backlight texture shapes if enabled
    if (this.enableBacklight && renderer.setTextureShapes && renderer.setTexturePaletteData) {
      const backlightShapes = this.createBacklightTextureShapes(renderer);
      const { highRes, lowRes } = this.generateBacklightPalettes(PLOT_SAMPLES, RENDER_SAMPLES);

      renderer.setTextureShapes(backlightShapes);
      renderer.setTexturePaletteData(highRes, lowRes);
    }
  }

  /**
   * Create texture shape for backlight emission from circle-grid pattern
   */
  protected createBacklightTextureShapes(renderer: {
    getMaskIndex: (name: string) => {
      msdfArrayIndex: number;
      msdfLayerIndex: number;
      alphaArrayIndex: number;
      alphaLayerIndex: number;
      hasMsdf: boolean;
      hasAlpha: boolean;
    };
    getMaskDimensions: (name: string) => { width: number; height: number };
  }): GPUTextureShape[] {
    // Get circle-grid mask info
    const maskIdx = renderer.getMaskIndex('circle-grid');
    const maskDims = renderer.getMaskDimensions('circle-grid');

    // Create fullscreen texture shape with circle-grid pattern
    const canvas = document.querySelector('canvas');
    const width = canvas?.width ?? 1280;
    const height = canvas?.height ?? 720;

    const backlightShape: GPUTextureShape = {
      x: 0,
      y: 0,
      width,
      height,
      layer: 0, // Background layer
      transmissionIndex: 0, // Full transmission (no absorption)
      emissionIndex: 0, // Use palette index 0 for emission
      reflectionIndex: 0, // No reflection (backlight doesn't reflect)
      msdfArrayIndex: maskIdx.msdfArrayIndex,
      msdfLayerIndex: maskIdx.msdfLayerIndex,
      alphaArrayIndex: maskIdx.alphaArrayIndex,
      alphaLayerIndex: maskIdx.alphaLayerIndex,
    ...
    return [backlightShape];
  }

  /**
   * Generate backlight palette data based on current backlight mode
   * Packed RGBA: R=transmission, G=emission, B=reflection, A=unused
   */
  protected generateBacklightPalettes(
    plotSamples: number,
    renderSamples: number
  ): { highRes: Float32Array[]; lowRes: Float32Array[] } {
    // Generate emission spectrum based on backlight mode
    const generateEmissionSpectrum = (
      samples: number,
      minWl: number,
      maxWl: number
    ): Float32Array => {
      const data = new Float32Array(samples * 4); // RGBA packed
      const step = (maxWl - minWl) / samples;

      for (let i = 0; i < samples; i++) {
        const wavelength = minWl + i * step;

        // Default: full transmission, no emission, no reflection
        const transmission = 1.0;
        let emission = 0.0;
        const reflection = 0.0;

        // Emission based on backlight mode
        if (this.backlightMode === 'normal') {
          // Visible light emission (D65-like)
          if (wavelength >= 380 && wavelength <= 700) {
            emission = 1.0;
          } else if (wavelength < 380 && wavelength > 250) {
            // UV falloff
            const t = (wavelength - 250) / (380 - 250);
            emission = 1.0 - (1.0 - t) * (1.0 - t);
          } else if (wavelength > 700 && wavelength < 850) {
            // IR falloff
            const t = (wavelength - 700) / (850 - 700);
            emission = 1.0 - t * t;
          }
        } else if (this.backlightMode === 'uv') {
          // UV light emission only
          if (wavelength >= 100 && wavelength < 150) {
            const t = (wavelength - 100) / 50;
            emission = 1.0 - (1.0 - t) * (1.0 - t);
          } else if (wavelength >= 150 && wavelength <= 350) {
            emission = 1.0;
          } else if (wavelength > 350 && wavelength < 380) {
            const t = (wavelength - 350) / (380 - 350);
            emission = 1.0 - t * t;
          }
          // Zero emission in visible range
        }
        // Dark mode: emission stays 0

        data[i * 4 + 0] = transmission;
        data[i * 4 + 1] = emission;
        data[i * 4 + 2] = reflection;
    ...
    const highRes = [generateEmissionSpectrum(plotSamples, 100, 1000)];
    const lowRes = [generateEmissionSpectrum(renderSamples, 100, 1000)];
