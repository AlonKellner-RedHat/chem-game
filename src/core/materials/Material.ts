...
export interface Material {
  ...
  /**
   * Reflection ratio (0-1). Default: 0.15
   * Controls the magnitude of reflection relative to transmission.
   * reflection = transmission * reflectionRatio
   * Higher values = brighter reflection, lower = dimmer.
   */
  reflectionRatio?: number;

  /**
   * Optional custom reflection spectrum generator.
   * If provided, completely overrides the default transmission-based reflection.
   * Use this for materials with special reflection properties (metals, etc).
   */
  ...
  /**
   * Generate reflection spectrum for ambient light simulation
   *
   * Unlike transmission, reflection is NOT depth-dependent. It represents
   * the fraction of incident ambient light that is reflected at each wavelength.
   *
   * Materials with high absorption typically have high reflection (Fresnel effect).
   * This is used for ambient light pass to give opaque materials visible color.
   *
   * @param wavelengthMin - Minimum wavelength (nm)
   * @param wavelengthMax - Maximum wavelength (nm)
   * @param resolution - Number of samples
   * @param properties - Material properties with mole fractions
   * @returns Reflection spectrum (0-1 for each wavelength)
   */
...
export function createMaterial(
  ...
  return {
    ...
    generateReflectionSpectrum(
      ...
      // This avoids Beer-Lambert which becomes numerically unstable for
      // highly absorptive materials (transmission → 0 everywhere).
      //
      // The reflected color is determined by "which wavelengths are absorbed less"
      // We compute: reflectivity ∝ 1 / (1 + k * extinction)
      // where k is a scaling factor to give reasonable contrast.
      //
      // This works correctly for any concentration, from dilute to opaque.
      ...
      // Calculate extinction-based reflectivity at each wavelength
      const extinctionSpectrum = new Float32Array(resolution);
      let maxExtinction = 0;
      ...
      for (let i = 0; i < resolution; i++) {
        ...
        let totalWeightedExtinction = 0;

        // Sum weighted extinction from all molecules
        for (const molecule of molecules) {
          const moleFraction = fractions[molecule.id] || 0;
          const legacyConc = concentrations[molecule.id] || 0;
        ...
        extinctionSpectrum[i] = totalWeightedExtinction;
        if (totalWeightedExtinction > maxExtinction) {
          maxExtinction = totalWeightedExtinction;
        }
      }

      // Convert extinction to reflectivity: wavelengths with less absorption reflect more
      // reflectivity = 1 / (1 + k * extinction)
      // k is chosen so that the peak extinction gives reasonable contrast
      const k = maxExtinction > 0 ? 2.0 / maxExtinction : 1.0;

      let maxReflectivity = 0;
      for (let i = 0; i < resolution; i++) {
        const reflectivity = 1.0 / (1.0 + k * extinctionSpectrum[i]);
        spectrum[i] = reflectivity;
        if (reflectivity > maxReflectivity) {
