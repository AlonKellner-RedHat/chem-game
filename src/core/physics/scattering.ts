 ...
 */
export function getRayleighScattering(
  wavelengthNm: number,
 ...
 * @returns Scattering coefficient (per cm)
 */
export function getMieScattering(
  wavelengthNm: number,
  params: MieParams
): number {
  if (params.particleDensity <= 0 || params.particleSize <= 0 || wavelengthNm <= 0) {
...
export function getScatteredFraction(
  ...
  const transmitted = applyScattering(1.0, wavelengthNm, smallParticleDensity, largeParticleDensity, pathLength);
