...
export function integrateVoigtOverBin(
  ...
  const voigtFWHM = 0.5346 * lorentzianFWHM + 
  ...
  const distanceFromPeak = Math.min(
    Math.abs(binStart - center),
    Math.abs(binEnd - center)
  );
  ...
  const numSteps = Math.max(
    minSteps,
    Math.ceil(binWidth / voigtFWHM * stepsPerFWHM)
  );
  ...
  let sum = voigtProfile(binStart - center, gaussianFWHM, lorentzianFWHM) +
